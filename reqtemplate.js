...
function scanNode(as, msg) {
    ...
    msg.getRequestHeader().setMethod("{ReqMethod}");
    msg.getRequestBody().setBody('{ReqBody}')
    if (header.getMethod() == 'GET') {
        ...
        //add headers
        if ({addedHeaders}) {
            reqHeaders = {{NucHeader}};
            for (key in reqHeaders) {
                msg.getRequestHeader().setHeader(key,reqHeaders[key]);
            }
        }
        
        //Tests for every path
        for(i = 0; i < URIlist.length; i++){
            
            if (stopScan){
                return
            }
            currentURI = URIlist[i]
            ...
            if ({condition}){
                //stop-at-first-match condition
                if ({stop-at-first-match}){
                    stopScan = true;
                }
